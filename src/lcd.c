...
short drawPos[2] = {screenWidth/2 , screenHeight/2}, controlPos[2] = {(screenWidth/2)-1, (screenHeight/2)-1};
short colVelocity = 1;
...
void position_update_ball()
{
  ...
  if (switches & SW3 && controlPos[0] > colLimits[0]){ 
	  controlPos[0] -= 1;
  }
  if (switches & SW4 && (controlPos[1] < colLimits[1])){ 
	  controlPos[0] += 1;
  }
}
